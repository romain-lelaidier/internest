...
class BinFileHandler(FileSystemEventHandler):
    ...
    def _wait_and_queue(self, path: Path):
        ...
        logger.info(f"Queued: {path.name} (MAC: {info.mac_address}, {info.duration_seconds:.2f}s)")
...
class BinFileWatcher:
    ...
    def __init__(
        ...
        # Mapping MAC address -> ESP ID (assigned dynamically)
        self.mac_to_esp_id: Dict[str, int] = {}
        self._next_esp_id = 1
    ...
    def get_esp_id(self, mac_address: str) -> int:
        """Get or assign an ESP ID for a MAC address."""
        if mac_address not in self.mac_to_esp_id:
            self.mac_to_esp_id[mac_address] = self._next_esp_id
            logger.info(f"Nouvel ESP détecté: {mac_address} -> ESP {self._next_esp_id}")
            self._next_esp_id += 1
        return self.mac_to_esp_id[mac_address]
    ...
    def _queue_existing_files(self):
        ...
        for path in existing_files:
            ...
            # Register MAC address to get ESP ID
            self.get_esp_id(info.mac_address)

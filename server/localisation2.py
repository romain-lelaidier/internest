...
def extract_patch(samples, id, box):
    sample = samples[id]
...
def box_tdoa(samples, box, id1, id2):
    s1 = extract_patch(samples, id1, box)
    s2 = extract_patch(samples, id2, box)
...
def localiser(esps, t1, t2):
    ...
    for mac, esp in esps.items():
        ...
        for i in range(num_features):
            ...
            boxes[id].append((t[x_min], t[x_max], f[y_min], f[y_max]))
    ...
    for box in agg_boxes:
        tdoas = []
        for i, (idi, si) in enumerate(samples.items()):
            for j, (idj, sj) in enumerate(samples.items()):
                if i >= j: continue
                # computing tdoa
                tdoa, cc = box_tdoa(samples, box, idi, idj)
                # removing values that are incoherent with the geometry
                if np.abs(tdoa) > 1.2 * np.linalg.norm(esps[idi].position - esps[idj].positions): continue
                tdoas.append((idi, idj, tdoa))

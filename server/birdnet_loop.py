...
def _has_activity(samples):
    """Vérifie si le buffer int16 contient de l'activité audio (pas juste du silence/bruit de fond)."""
    if len(samples) == 0:
        return False
    rms = np.sqrt(np.mean(samples.astype(np.float64) ** 2))
    return rms > CONFIG.ACTIVITY_RMS_THRESHOLD


def _esp_loop_2(mac, esp):
    """
    Boucle BirdNET secondaire : fenêtres de 30s, analyse uniquement s'il y a de l'activité.
    Complémentaire à _esp_loop (fenêtres courtes en continu).
    """
    active_species = {}
    ...
    while True:
        ...
        unanalyzed_us = esp.buffer_end_t - last_analyzed_t
        if unanalyzed_us < CONFIG.BIRDNET_WINDOW_2_S * 1e6:
            # vérifier les départs en attendant
            now = time.time()
            departed = [sp for sp, ts in active_species.items() if now - ts > CONFIG.SPECIES_TIMEOUT_S]
            for sp in departed:
                print(f"[L2] <<< {sp} partie de {mac}")
                if CONFIG.AFFICHAGE_IHM:
                    notify_departure(mac, sp)
                del active_species[sp]
            time.sleep(CONFIG.POLL_INTERVAL_S)
        ...
        # lire la fenêtre de 30s
        t1 = last_analyzed_t
        t2 = t1 + CONFIG.BIRDNET_WINDOW_2_S * 1e6
        _, _, samples = esp.read_window(t1, t2)
        ...
        # vérifier l'activité avant d'analyser
        if not _has_activity(samples):
            print(f"[L2] Pas d'activite sur {mac}, skip")
            continue
        ...
        try:
            ...
            print(f"[L2] Erreur BirdNET pour {mac}: {e}")
